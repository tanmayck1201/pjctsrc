...
class Thread(QThread):
    ...
    def test_image_with_ela(self, image_path, model_path):
        ...
        """
        self.model_path = r"C:\Users\Tanmay\Documents\Major_Pjct\DFSpot-Deepfake-Recognition\img_splice\sys_models\ELA_Model.h5"
        # loading Model
    ...
    def test_image_with_vgg16(self, image_path, model_path):
        """
                ...
                :return label[Forged , Not Forged] , prob[class probability]
        """
        self.model_path = r"C:\Users\Tanmay\Documents\Major_Pjct\DFSpot-Deepfake-Recognition\img_splice\sys_models\VGG16_Model.h5"
        model = load_model(model_path)
        # Read image
        image = Image.open(image_path).convert('RGB')
    ...
    def test_image_with_vgg19(self, image_path, model_path):
        ...
        """
        self.model_path = r"C:\Users\Tanmay\Documents\Major_Pjct\DFSpot-Deepfake-Recognition\img_splice\sys_models\VGG19_Model.h5"
        model = load_model(model_path)
        # Read image
        image = Image.open(image_path).convert('RGB')
...
class Test_window(QWidget):
    def __init__(self, parent=None, model_path=r"C:\Users\Tanmay\Documents\Major_Pjct\DFSpot-Deepfake-Recognition\img_splice\sys_models\ELA_Model.h5"):
        super().__init__()
    ...
    def init_window(self):
        ...
        # icon Pic File name
        self.setWindowIcon(QtGui.QIcon("Icons/icons8-cbs-512.ico"))
        ...
        # Notice using h2 tags From Html
        self.button.setToolTip(
            "<h5>Browse image from your computer to start test!<h5>")
        # icon Pic File name
        ...
        # Notice using h2 tags From Html
        self.button.setToolTip(
            "<h5>test image either Forged or Not Forged!<h5>")
        # icon Pic File name
        self.button.setIcon(QtGui.QIcon(
        ...
        self.button.setToolTip(
            "<h5>test image either Forged or Not Forged!<h5>")
        # icon Pic File name
        self.button.setIcon(QtGui.QIcon("Icons/repeat-pngrepo-com.png"))
        self.button.setIconSize(QtCore.QSize(15, 15))  # to change icon Size
        ...
        # Notice using h2 tags From Html
        self.button.setToolTip("<h5>Close the program!<h5>")
        # icon Pic File name
        self.button.setIcon(QtGui.QIcon(
    ...
    @pyqtSlot()
    def getfiles(self):
        fileName, extention = QFileDialog.getOpenFileName(
            self, 'Single File', 'C:\'', "*.png *.xpm *.jpg *.tiff *.jpg *.bmp")
    ...
    def on_click(self):
        if self.file_path == "":
            ...
            if str(self.combo.currentText()) == "Error Level Analysis":
                model = r"C:\Users\Tanmay\Documents\Major_Pjct\DFSpot-Deepfake-Recognition\img_splice\sys_models\ELA_Model.h5"
                self.myThread = Thread()
                label, prob = self.myThread.test_image_with_ela(
                    self.file_path, model_path=model)
                self.myThread.start()
                self.close()
                ...
                model = r"C:\Users\Tanmay\Documents\Major_Pjct\DFSpot-Deepfake-Recognition\img_splice\sys_models\VGG16_Model.h5"
                self.myThread = Thread()
                label, prob = self.myThread.test_image_with_vgg16(
                    self.file_path, model_path=model)
                self.myThread.start()
                self.close()
                self.result_window = ResultWindow(label, prob)
                ...
                model = r"C:\Users\Tanmay\Documents\Major_Pjct\DFSpot-Deepfake-Recognition\img_splice\sys_models\VGG19_Model.h5"
                self.myThread = Thread()
                label, prob = self.myThread.test_image_with_vgg19(
                    self.file_path, model_path=model)
                self.myThread.start()
                self.close()
                self.result_window = ResultWindow(label, prob)
